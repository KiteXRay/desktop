...
export const ProfileCard: React.FC<ProfileCardProps> = React.memo(({
  ...
  return (
    <div
      ...
      onDragEnd={onDragEnd}
      className={`group relative rounded-xl transition-all duration-150 cursor-pointer border select-none ${
        compact ? 'p-2' : 'p-2.5'
      } ${
        isDragging
    ...
    >
      {compact ? (
        /* Compact Mode: Streamlined single-row layout */
        <div className="flex items-center justify-between gap-2 min-h-[30px]">
          {/* Left: Status Indicator, Label, Protocol & Sub badge */}
          <div className="flex items-center gap-2 min-w-0 flex-1">
            ...
            <span className="text-xs font-medium text-slate-100 truncate" title={label}>
              {label}
            </span>
          ...
          <div className="flex items-center gap-1.5 shrink-0" onClick={e => e.stopPropagation()}>
            ...
            <button
              onClick={onPing}
              disabled={isPinging}
              className={`flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-mono transition-colors border cursor-pointer ${
                isPinging
        ...
        /* Comfortable / Standard 3-Row View */
        <>
          ...
          {/* Row 3: Protocol Tags & Reset/Edit/Remove buttons */}
